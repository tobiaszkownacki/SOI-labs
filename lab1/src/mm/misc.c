...
void maxChildren(int* children, pid_t* who, int A, int B)
{
  int maxChildren = -1;
  pid_t found = -1;
  int count = 0;
  ...
  for (proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr)
  {
	if (mproc[proc_nr].mp_flags & IN_USE)
	{
		...
		if (count > maxChildren && count >= A && count <= B)
		{
			maxChildren = count;
...
int countChildrenAtLevels(int proc_nr, int N)
{
	int children = 0;
	int i = 0;
	if(N < 0)
	...
	for(i = 0; i < NR_PROCS; ++i)
	{
		if((mproc[i].mp_flags & IN_USE) && proc_nr != i && mproc[i].mp_parent == proc_nr)
		{
			...
			if(N > 0)
			{
				children += countChildrenAtLevels(i, N - 1);
			}
		}
	}
...
void maxChildrenWithinLevels(int* children, pid_t* who, int N)
{
	int maxChildren = -1;
	pid_t found = -1;
	int count = 0;
	...
	for (proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr)
	{
		if (mproc[proc_nr].mp_flags & IN_USE)
		{
			count = countChildrenAtLevels(proc_nr, N - 1);
...
PUBLIC int do_maxChildrenWithinLevels()
{
	int children = -1;
	pid_t who = -1;
	int N = mm_in.m1_i1;
	maxChildrenWithinLevels(&children, &who, N);
	return children;
}

PUBLIC int do_whoMaxChildrenWithinLevels()
{
	int children = -1;
	pid_t who = -1;
	int N = mm_in.m1_i1;
